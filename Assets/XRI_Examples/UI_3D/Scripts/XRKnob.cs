...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    /// Đã chỉnh sửa để xoay vô lăng được đa điểm
    /// </summary>
    public class XRKnob : XRBaseInteractable
    {
        ...
        void StartGrab(SelectEnterEventArgs args)
        {
            ...
            Debug.Log(m_Interactor[0].transform.gameObject.name);
        ...
        {
            m_Interactor.Remove(args.interactorObject);

            if (m_Interactor.Count > 0)
            {
                m_PositionAngles.Reset();
                m_UpVectorAngles.Reset();
                m_ForwardVectorAngles.Reset();
        ...
        void UpdateRotation(bool freshCheck = false)
        {
            ...
            var interactorTransform = m_Interactor[0].GetAttachTransform(this);
